...
rho_dt = read_csv("rhomat_from_superpop_4cat.csv")
rho_dt

icc_dt = read_csv("rankICCs_from_superpop_4cat.csv")
icc_dt
...
rho_dt_main_fmt = rho_dt_main %>% filter(beta_X_list==log(1.5)) %>% 
  ...
  mutate_at(vars(gamma0:rho22),~ sprintf("%.4f", .x))

rho_dt_main_fmt %>% write_csv("from_superpop_4cat_fmt.csv")
